...
# ---------------------------------------------------------------------------
# SGP4/TLE 示例两行根数（国际空间站 ISS，NORAD 25544）
# 本星座配置中演示如何通过 OrbitalElements.from_tle() 构造 TLE 驱动的卫星。
# 实际生产部署可由 Celestrak 导入实时 TLE 替换此处占位符。
# ---------------------------------------------------------------------------
_TLE_ISS_L1 = "1 25544U 98067A   24001.50000000  .00006088  00000+0  11281-3 0  9993"
_TLE_ISS_L2 = "2 25544  51.6416 245.5985 0002917 140.8660  18.8238 15.49559112432578"

# 预定义的低轨卫星星座（减少到8颗以提高性能）
# ⭐ 每颗卫星都具备上述三种天线配置
# ⭐ TLE 示例：遥感一号使用真实 TLE 格式驱动 SGP4 传播；其余走二体回退
LEO_SATELLITES = [
    # 遥感一号：演示 TLE/SGP4 路径（真实 ISS TLE 作为占位符，实际应替换为真实星历）
    OrbitalElements.from_tle(
        "遥感一号", "LEO_001",
        _TLE_ISS_L1, _TLE_ISS_L2,
    ),
    # 以下卫星使用经典二体开普勒解析解（无 TLE）
